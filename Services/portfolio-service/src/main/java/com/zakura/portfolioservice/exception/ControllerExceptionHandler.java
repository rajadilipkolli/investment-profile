...
@Component
@Slf4j
@ControllerAdvice("com.zakura.portfolioservice")
public class ControllerExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public final ResponseEntity<StockError> handleRestControllerException(RestControllerException e,
			WebRequest request) {
	...
	private ResponseEntity<StockError> handleUnexpectedException(RestControllerException e, WebRequest request) {
	...
	private ResponseEntity<StockError> errorResponse(RestControllerException e, HttpHeaders headers) {
		return new ResponseEntity<>(new StockError(Integer.valueOf(e.getHttpStatus().value()), e.getMessage()), headers,
				e.getHttpStatus());
	}
