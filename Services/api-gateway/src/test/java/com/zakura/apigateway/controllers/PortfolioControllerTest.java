...
public class PortfolioControllerTest {
	...
	public void testAllInvestments() throws Exception {
		...
		mockMvc.perform(get("/restservices//investments/all").contentType(MediaType.APPLICATION_JSON_VALUE)
	...
	public void testUpdateInvestment() throws Exception {
		...
		Mockito.when(jwtUtils.getUserNameFromJwtToken(Mockito.anyString())).thenReturn(TestData.USER_ID);
		Mockito.when(portfolioServiceClient.updateUserInvestment(Mockito.any(), Mockito.anyString()))
				.thenReturn(TestData.getInvestment());
		mockMvc.perform(post("/restservices/investments/update", new Object[] { TestData.USER_ID }).content(body)
	...
	public void testDeleteInvestment() throws Exception {
		Mockito.when(jwtUtils.getUserNameFromJwtToken(Mockito.anyString())).thenReturn(TestData.USER_ID);
		Mockito.when(portfolioServiceClient.deleteUserInvestment(Mockito.any(), Mockito.anyString()))
				.thenReturn(TestData.SUCCESS);
		final String body = TestData.getInvestmentString();
		mockMvc.perform(post("/restservices/investments/delete").content(body).param("request", body)
	...
	public void testProfitInvestments() throws Exception {
		...
		mockMvc.perform(get("/restservices/investments/profit").contentType(MediaType.APPLICATION_JSON_VALUE)
				.header(TestData.AUTHORIZATION_STRING, TestData.AUTHORIZATION_HEADER_VALUE))
				.andDo(MockMvcResultHandlers.print()).andExpect(status().isOk()).andReturn();
	...
	public void testLossInvestments() throws Exception {
		...
		mockMvc.perform(get("/restservices/investments/loss").contentType(MediaType.APPLICATION_JSON_VALUE)
				.header(TestData.AUTHORIZATION_STRING, TestData.AUTHORIZATION_HEADER_VALUE))
				.andDo(MockMvcResultHandlers.print()).andExpect(status().isOk()).andReturn();
	}
