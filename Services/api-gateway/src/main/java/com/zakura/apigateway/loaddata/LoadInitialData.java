...
public class LoadInitialData implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        this.reactiveRoleRepository.deleteAll().thenMany(Flux.fromIterable(roles).flatMap(this.reactiveRoleRepository::save).log()).then(this.reactiveUserRepository.deleteAll()).then(getUserMono().flatMap(this.reactiveUserRepository::save)).subscribe(data -> log.info("data: {}", data), err -> log.error("error:", err), () -> log.info("data initialization done..."));
    }

    private Mono<User> getUserMono() {
        Set<Role> roleSet = new HashSet<>();
        return this.reactiveRoleRepository.findByName(ERole.ROLE_USER).map(roleSet::add).flatMap(aBoolean -> {
            User user = new User();
