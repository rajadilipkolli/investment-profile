...
public class StockService {
    ...
    public StockService(PortfolioServiceClient portfolioServiceClient, StockRepository stockRepository) {
    ...
    public String saveUserStock(StockRequest stockRequest, String userId) {
        ...
        BigDecimal currentPrice = stockRequest.currentPrice().divide(BigDecimal.valueOf(100), RoundingMode.HALF_UP)
                .multiply(BigDecimal.valueOf(profitLossPercent)).add(stockRequest.currentPrice());
        currentPrice = currentPrice.setScale(2, RoundingMode.HALF_UP);
        boolean isProfit = true;
        Investment investmentToSave = Investment.builder().costPrice(stockRequest.currentPrice())
                .currentPrice(currentPrice).name(stockRequest.name()).quantity(stockRequest.quantity())
                .type(stockRequest.investmentType()).profitLossPercent(profitLossPercent).profit(isProfit).build();
    ...
    public Stock saveStock(StockRequest stockRequest) {
        return this.stockRepository.save(Stock.builder()
